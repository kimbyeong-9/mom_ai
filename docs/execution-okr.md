...
- **기능**: (2026-07-22 구현 완료 — 결정로그 03의 2026-07-22 개정 참고)
  - Automation 페이지 — 연결된 자동화 목록
  - 채용/원격구인 모니터링: n8n은 스케줄에 맞춰 백엔드 내부 엔드포인트
    (`POST /internal/automation-checks/run`)를 호출하는 역할만 하고, 실제 검색·중복 제거·
    매칭 계산은 백엔드(`AutomationsService`)가 전담 — 웹훅 콜백 왕복 대신 단방향 호출로 단순화
  - 실행 상태 표시 — 1회성 완료가 아니라 "지속 실행 중" 상태와 마지막 확인 시각·매칭 건수를
    실제 데이터로 표시 (`AutomationLiveMonitorCard`)
  - 비자·체류 마감일 리마인더는 n8n의 일일 스케줄 트리거로 D-7/D-1/D-0에 Gmail SMTP를 통해
    실제 이메일 발송까지 완료 (푸시 알림은 다음 확장 후보)
- **성공 기준**: 실행율 70% 이상
